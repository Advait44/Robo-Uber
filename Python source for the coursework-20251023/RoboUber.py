...
def runRoboUber(worldX, worldY, runTime, stop, junctions=None, streets=None, interpolate=False, outputValues=None,
    ...
    print(f"Starting world - logging data to {log_filename}")

    # Open the CSV file for writing
    with open(log_filename, 'w') as log_file:
        # Write the header row
        log_file.write("Time,T1_Balance,T2_Balance,T3_Balance,T4_Balance,Dispatcher_Revenue,Active_Taxis\n")
        ...
        while threadTime < threadRunTime:
            ...
            if stop.is_set():
                ...
                try:
                    t1_bal = taxis[0]._account
                    t2_bal = taxis[1]._account
                    t3_bal = taxis[2]._account
                    t4_bal = taxis[3]._account

                    # If dispatcher.py uses a different name, default to 0 to prevent crash
                    if hasattr(dispatcher0, '_account'):
                        disp_rev = dispatcher0._account
                    elif hasattr(dispatcher0, 'balance'):
                        disp_rev = dispatcher0.balance
                    else:
                        disp_rev = 0

                        # Count how many taxis are still working (account > 0)
                    active_count = sum(1 for t in taxis if t._account > 0)

                    # Write to file
                    log_file.write(f"{threadTime},{t1_bal},{t2_bal},{t3_bal},{t4_bal},{disp_rev},{active_count}\n")
                ...
                if threadTime != svcArea.simTime:
                    threadTime += 1
                time.sleep(0.001)  # Speed up simulation for data collection
...
for run in range(numDays):
    ...
    roboUber = threading.Thread(target=runRoboUber,
                                ...
                                kwargs={'worldX': worldX,
                                        ...
                                        'fareFile': fareFile,
                                        'currentDay': run})  # <--- ADDED 'currentDay' HERE
